...
internal partial class Request
{
    ...
    {
        internal Azure.Files.Emulator.ShareNameRestypeShareCompFilepermission.ShareCreatePermission.Requests.ApplicationXml? ApplicationXml { get; private set; }
        ...
        internal static async Task<RequestContent> BindAsync(HttpRequest request, CancellationToken cancellationToken)
        {
            ...
            switch (requestContentMediaType?.MediaType?.ToLower())
            {
                case "application/xml":
                    ...
                    {
                        ApplicationXml = (await Azure.Files.Emulator.HttpRequestExtensions.BindBodyAsync<Azure.Files.Emulator.ShareNameRestypeShareCompFilepermission.ShareCreatePermission.Requests.ApplicationXml>(request, cancellationToken).ConfigureAwait(false)).AsOptional()
                    };
                default:
